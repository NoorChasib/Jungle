...
RSpec.describe User, type: :model do
			...
			it 'gives appropriate error if password confirmation does not match password' do
				@user =
					User.create(
						...
						email: 'noor@noor.com',
						password: 'password',
						password_confirmation: 'paswordd',
					)
				expect(@user.errors.full_messages).to_not be_empty
				expect(@user.errors.full_messages[0]).to eql(
	...
	describe '.authenticate_with_credentials' do
		...
		it 'returns the correct user object given the needed information' do
			expect(
				User.authenticate_with_credentials('noor@noor.com', 'password'),
			).to eql(User.last)
		end

		it 'returns nil if the email is not found' do
			expect(
				User.authenticate_with_credentials('noor@nor.com', 'password'),
			).to be_nil
		end

		it 'returns nil if the password is incorrect' do
			expect(
				User.authenticate_with_credentials('noor@noor.com', 'passwor'),
			).to be_nil
		end
